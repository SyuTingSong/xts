    ...
    class HailStone extends RainTPL {
        ...
        protected function check_template($tpl_name) {
            ...
            if(!is_writable(self::$compile_dir))
                throw new RainTpl_Exception ('Cache directory ' . self::$compile_dir . 'doesn\'t have write permission. Set write permission or set RAINTPL_CHECK_TEMPLATE_UPDATE to false. More details on http://www.raintpl.com/Documentation/Documentation-for-PHP-developers/Configuration/');
...
namespace xts {
    require_once(X_LIB_ROOT.'/view.php');
    use \HailStone;

    class Hail extends Component implements View {
        /**
         * @var HailStone
         */
        private $hail;
        ...
        public function render($tpl, $params=array(), $cacheId=null) {
            echo $this->renderFetch($tpl, $params, $cacheId);
            return $this;
        }
        ...
        public function renderFetch($tpl, $params=array(), $cacheId=null) {
            $useCache = !is_null($cacheId) && $this->conf['cacheId'];
            if($useCache) {
                /**
                 * @var \xts\Cache $cache
                 */
                $cache = XComponentFactory::getComponent($this->conf['cacheId']);
